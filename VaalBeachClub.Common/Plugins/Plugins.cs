...
namespace VaalBeachClub.Common.Plugins
{
   public class PluginManager
    {
        #region Fields

        private static readonly IVaalBeachClubFileProvider _fileProvider;
        ...
        static PluginManager()
        {
            ...
            var refsPathName = _fileProvider.Combine(Environment.CurrentDirectory, VaalBeachClubPluginDefaults.RefsPathName);
        ...
        private static IEnumerable<KeyValuePair<string, PluginDescriptor>> GetDescriptionFilesAndDescriptors(string pluginFolder)
        {
            ...
            foreach (var descriptionFile in _fileProvider.GetFiles(pluginFolder, VaalBeachClubPluginDefaults.DescriptionFileName, false))
            {
        ...
        private static IList<string> GetInstalledPluginNames(string filePath)
        {
            ...
            if (!_fileProvider.FileExists(filePath))
            {
                ...
                filePath = _fileProvider.MapPath(VaalBeachClubPluginDefaults.ObsoleteInstalledPluginsFilePath);
                if (!_fileProvider.FileExists(filePath))
                ...
                SaveInstalledPluginNames(pluginSystemNames, _fileProvider.MapPath(VaalBeachClubPluginDefaults.InstalledPluginsFilePath));
        ...
        private static Assembly PerformFileDeploy(string plug, ApplicationPartManager applicationPartManager, VaalBeachClubConfig config, string shadowCopyPath = "")
        {
        ...
        /// <param name="plug">Plugin file info</param>
        /// <returns>Assembly</returns>
        private static Assembly RegisterPluginDefinition(VaalBeachClubConfig config, ApplicationPartManager applicationPartManager, string plug)
        {
        ...
        private static bool IsPackagePluginFolder(string folder)
        {
            ...
            if (!_fileProvider.GetDirectoryNameOnly(parent).Equals(VaalBeachClubPluginDefaults.PathName, StringComparison.InvariantCultureIgnoreCase))
                return false;
        ...
        /// <param name="applicationPartManager">Application part manager</param>
        /// <param name="config">Config</param>
        public static void Initialize(ApplicationPartManager applicationPartManager, VaalBeachClubConfig config)
        {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                var pluginFolder = _fileProvider.MapPath(VaalBeachClubPluginDefaults.Path);
                _shadowCopyFolder = _fileProvider.MapPath(VaalBeachClubPluginDefaults.ShadowCopyPath);
                _reserveShadowCopyFolder = _fileProvider.Combine(_fileProvider.MapPath(VaalBeachClubPluginDefaults.ShadowCopyPath), $"{VaalBeachClubPluginDefaults.ReserveShadowCopyPathName}{DateTime.Now.ToFileTimeUtc()}");
                ...
                try
                {
                    var installedPluginSystemNames = GetInstalledPluginNames(_fileProvider.MapPath(VaalBeachClubPluginDefaults.InstalledPluginsFilePath));
                    ...
                    if (config.ClearPluginShadowDirectoryOnStartup)
                    {
                        ...
                        foreach (var directory in _fileProvider.GetDirectories(_shadowCopyFolder, VaalBeachClubPluginDefaults.ReserveShadowCopyPathNamePattern))
                        {
                            try
                            {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains("4.1", StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        public static void MarkPluginAsInstalled(string systemName)
        {
            ...
            var filePath = _fileProvider.MapPath(VaalBeachClubPluginDefaults.InstalledPluginsFilePath);
        ...
        public static void SavePluginDescriptor(PluginDescriptor pluginDescriptor)
        {
            ...
            var filePath = _fileProvider.Combine(_fileProvider.GetDirectoryName(pluginDescriptor.OriginalAssemblyFile), VaalBeachClubPluginDefaults.DescriptionFileName);
            if (!_fileProvider.FileExists(filePath))
        ...
        public static string GetLogoUrl(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = VaalBeachClubPluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{VaalBeachClubPluginDefaults.LogoFileName}.{ext}")));
            ...
            var logoUrl = $"{webHelper.GetStoreLocation()}plugins/{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{VaalBeachClubPluginDefaults.LogoFileName}.{logoExtension}";
            return logoUrl;
        }
